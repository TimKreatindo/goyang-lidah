							...
							html += `<tr>
										<td>
											<input type="text" name="organisasi[]" id="organisasi" class="form-control" required
												placeholder="Nama Organisasi..." value="${d.data[i]}">
										</td>
...
function load_table() {
	...
	$("#main-table").dataTable({
		...
		order: [],
		ajax: {
			url: base_url + "admin/datatable-organisasi",
			type: "POST",
		},
		columnDefs: [
			{
			...
			setTimeout(() => {
				...
				if (d.status == false) {
					...
					if (d.status == false) {
						...
						let html_bukti = "";
						const base_file = base_url + "assets/upload/organisasi/";
						if (bukti.type == "file") {
